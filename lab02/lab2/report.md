...
\[
T(x,0) = T_{\text{init}}, \quad T(0,t) = T_{\text{left}}, \quad T(L,t) = T_{\text{right}}.
\]

## Метод решения
Используется неявная конечно-разностная схема (метод прогонки). 
...
![Распределение температуры](screenshot_temp_profile.png)


## Выводы
1. Разработанная программа корректно реализует неявную конечно-разностную схему для уравнения теплопроводности.
2. Подтверждена сходимость численного решения при измельчении сетки.
3. Выбор оптимальных шагов сетки требует компромисса между точностью и временем счёта.
4. Программа может быть использована для учебных и исследовательских целей при изучении процессов теплопереноса.